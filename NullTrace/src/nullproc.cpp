...
bool NullProcess::Process::setProcByName(const std::string& pkg) {
    ...
    for(const auto &entry : fs::directory_iterator("/proc")) {
        ...
        } catch(const std::invalid_argument &e) { /*skip non pid entries*/ }
    }
    return pid != -1;
}
...
NullProcess::Process::writeProcessMemory(std::string hex, uintptr_t address) {
    ...
    if(!NullTrace::ptraceWrite(this->pid, address, hexVec.data(), hexVec.size()))
        return false;
...
bool NullProcess::Process::injectLibrary(std::string path) {
    ...
    std::cout << "[NullTrace] LIBC Malloc Address: " << "0x" << std::hex
              << this->libc.remote_malloc << " LIBC Free Address: " << "0x" << std::hex
              << this->libc.remote_free << " LIBDL Dlopen Address: " << "0x" << std::hex << this->libdl.remote_dlopen
              << " LIBDL Dlsym Address: " << "0x" << std::hex << this->libdl.remote_dlsym <<"\n";
    ...
    std::cout << "[NullTrace] libLR base: " << "0x" << std::hex << libLR_base << "\n";
