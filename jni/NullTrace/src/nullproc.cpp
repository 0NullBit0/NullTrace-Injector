...
bool NullProcess::Process::injectLibNB(void* pathAddr, uintptr_t libLR_base) {
    ...
    NullProcess::Map nbMap =this->findMap("libnativebridge.so");
    if(nbMap.pathName.empty()) {
        useCallbacks = true;
        nbMap = this->findMap("libhoudini.so");
    }
    if(nbMap.pathName.empty()) {
        ...
        nbMap = this->findMap(nbStr.data());

    }
    void* handle = nullptr;
    if(!nbMap.pathName.empty()) {

        uintptr_t nbLoadLibrary;
        uintptr_t nbLoadLibraryExt;
        uintptr_t nbCallbacks;

        if(useCallbacks) {
            nbCallbacks = (NullElf::getAddrSym(nbMap.pathName.c_str(), "NativeBridgeItf") + nbMap.start);
            if (!nbCallbacks)
                std::cerr << "[NullTrace] Failed finding callbacks!\n";
        }
        else {
            nbLoadLibrary = (NullElf::getAddrSym(nbMap.pathName.c_str(), "_ZN7android23NativeBridgeLoadLibraryEPKci") + nbMap.start);
            nbLoadLibraryExt = (NullElf::getAddrSym(nbMap.pathName.c_str(), "_ZN7android26NativeBridgeLoadLibraryExtEPKciPNS_25native_bridge_namespace_tE") + nbMap.start);
            std::cerr << "[NullTrace] Failed finding nb symbols!\n";
        }

        NullUtils::NativeBridgeCallbacks callbacks{};
        if(useCallbacks)
            callbacks = this->readProcessMemory<NullUtils::NativeBridgeCallbacks>(nbCallbacks);

        if(NullUtils::getApiLevel() >= 26) {
            if(useCallbacks)
                handle = this->callR<void *>(libLR_base,
                                         reinterpret_cast<uintptr_t>(callbacks.loadLibraryExt),
                                         pathAddr, RTLD_NOW | RTLD_GLOBAL, 3);
            else
                handle = this->callR<void *>(libLR_base, nbLoadLibraryExt,
                                             pathAddr, RTLD_NOW | RTLD_GLOBAL, 3);
        }else {
            if(useCallbacks)
                handle = this->callR<void*>(libLR_base, reinterpret_cast<uintptr_t>(callbacks.loadLibrary), pathAddr, RTLD_NOW | RTLD_GLOBAL);
            else
                handle = this->callR<void *>(libLR_base, reinterpret_cast<uintptr_t>(nbLoadLibrary),
